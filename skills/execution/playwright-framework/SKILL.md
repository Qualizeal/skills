...
description: Playwright framework architecture — project structure, config, fixtures, page objects, authentication and storage state, environment handling, parallelism and reporters. Use when setting up a Playwright suite, adding a fixture or page object, changing playwright.config, or reviewing framework structure.
---

# Playwright framework architecture

The framework's job is to make the *test* the only interesting part of a test file. Anything a reader has to skim past — login steps, seeding, waiting, env branching — belongs in the framework.
