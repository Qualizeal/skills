...
description: How to choose, write and review Playwright locators — the priority ladder, filtering and chaining, handling lists and dynamic content, strictness violations, iframes and shadow DOM, and adding test ids to the application. Use when writing or fixing a selector, reviewing test code, or when a locator is ambiguous or broken.
---

# Locator strategy

Locator choice determines how long a test survives. Everything else in a suite can be mediocre and it will still function; a suite built on structural selectors breaks on the next CSS refactor and gets abandoned.
