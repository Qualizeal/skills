---
name: playwright-mcp
description: Using the Playwright MCP server and Playwright CLI to drive a real browser for exploration, locator discovery, reproduction of defects and test scaffolding, including configuration, the snapshot loop, token cost and safety guardrails. Use when exploring an application live, discovering selectors, reproducing a reported bug, or converting an exploratory session into a spec file.
---
...
5. **Write the spec properly** — the exploration produces knowledge, not code. Author the test against `playwright-framework` and `playwright-script-generation`: fixtures for setup, page objects for locators, API-driven preconditions.
